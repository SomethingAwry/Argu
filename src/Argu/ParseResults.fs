...
type ParseResults<[<EqualityConditionalOn; ComparisonConditionalOn>]'Template when 'Template :> IArgParserTemplate>
    ...
    let getResult rs (e : Expr) =
        ...
        | None ->
            ...
            error (not aI.IsCommandLineArg) ErrorCode.PostProcess $"ERROR: missing argument '%s{aI.Name.Value}'."
        | Some r when restrictF rs r -> r
        | Some r -> error (not r.CaseInfo.IsCommandLineArg) ErrorCode.PostProcess $"ERROR: missing argument '%s{r.CaseInfo.Name.Value}'."

    let parseResult (f : 'F -> 'S) (r : UnionCaseParseResult) =
        try f (r.FieldContents :?> 'F)
        with e -> error (not r.CaseInfo.IsCommandLineArg) ErrorCode.PostProcess $"ERROR parsing '%s{r.ParseContext}': %s{e.Message}"
