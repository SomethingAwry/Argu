...
//)
Target.create "Clean" (fun _ -> !! "src/**/bin" ++ "src/**/obj" ++ "artifacts" |> Shell.cleanDirs)


// --------------------------------------------------------------------------------------
// Build library & test project

let setVersion (buildOptions: DotNet.BuildOptions) =
    { buildOptions with DotNet.BuildOptions.MSBuildParams.Properties = [("Version", release.NugetVersion)] }
...
Target.create "Build" (fun _ ->
    !! "src/**/*.*proj"
    |> Seq.iter (DotNet.build setVersion)
)
//Target.create "Build" (fun _ ->
//    !! "src/**/*.*proj"
...
//        }) __SOURCE_DIRECTORY__
//)
Target.create "RunTests" (fun _ -> !! "src/**/*.*proj" |> Seq.iter (DotNet.test id))
